...
triggers any => 'dessert', 'desserts', 'desert', 'deserts';

handle query_lc => sub{
	return unless my ($in) = $_ = /^a?\s*des?serts?\s+(?:that)?\s*(?:start|beginn?)s?(?:ing)?\s*(?:with)?\s+([a-zA-Z])$/;

=pod	my $output = 'A Desert beginning with ' . (uc $in) . ' is ';
	my @items;
	if($in eq "a"){
